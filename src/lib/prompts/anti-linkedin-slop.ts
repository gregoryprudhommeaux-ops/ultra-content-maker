...
const REGIONAL_FILTER: Record<ContentLanguage, string> = {
  ...
  es: `Distinguish Mexico vs Spain (never mix). Default Guadalajara/LA MESA = MX warm-pro B2B (tú, concrete). MX bans: vosotros, vale, venga, ordenador; prefer computadora, celular, platicar (when warm-pro fits), coordinar una reunión/llamada (not agendar), eliminar/quitar (not remover). Do not “localize” with street MX (güey/órale spam) or Spain colloquial (tío/mola). Ban fake español neutro (tú + distant formality + English -ing gerunds). ES-Spain (brief only): local direct register · avoid LATAM-only filler.`,
};
...
- Qualification triad (bullets OR inline prose): ban "Même industrie, même fonction, même irritant…" / "Same industry, same role, same pain" / "Misma industria, misma función…" and neat 3-bullet "same X / same Y / same Z" checklists · keep ONE sharp filter criterion in uneven prose; drop or demote the other two. **≠ lexical triplets** (*clair, direct, efficace*): same “triplet” family, two layers (framework vs surface rhythm) — flag both if present; neither absorbs the other.
...
- Obsolete networking metaphors (always ban): business cards / tarjetas de visita / cartes de visite / "colección de tarjetas". Prefer 2020s reality: cocktail pitch rooms, LinkedIn connects, weak first intros, everyone selling / nobody listening.
...
- School connectors: "Pour commencer / Tout d'abord / Premièrement / Enfin", "First and foremost / To begin with", "Para empezar" · paragraph order should carry the logic. **Overlap with numbered moral lists:** *Premièrement* as local school connector = school_opener; full "3 lessons / 1-2-3 moral arc" = numbered_moral_list — name both; do not collapse into one ID.
- Bullet UI slop: identical emoji on every bullet; systematic "**Bold lead:**" on every line · prefer plain "-" sometimes.
- Syntax tics (all languages · high detectability): excess em dashes (—) · max 1 per paragraph; HARD BAN "Ce n'est pas X, c'est Y" / "It's not about X, it's about Y" / "No es solo X, es Y" / "Il ne s'agit pas de X, mais de Y" · zero per post · state the claim without the antithesis formula; lexical triplets anywhere ("clair, direct, efficace") · distinct from qualification triad (see above); evenly sized paragraphs · vary hard (2-line then 8-line).
- Classic LinkedIn packaging (HARD BAN): years-legitimation openers ("J'ai passé X années…"); fake scoop ("Personne ne parle de ça"); closed rhetorical CTA closes ("Tu veux vraiment laisser passer ça ?") — distinct from one mid-body open question used for rhythm; fake-we intimacy ("On a tous vécu…"); round follower milestones ("1000 abonnés. Ce que j'ai appris"); "Simple mais puissant / Simple yet powerful"; 3+ anaphora stacks (Quand…/Si…/When…); vague coming-soon CTA ("Je prépare quelque chose / Something is coming"); false humility ("Je ne suis pas un expert, mais…"); self-title hook ("En tant que CEO/founder/thought leader…" / "As a serial entrepreneur…"); thread meta-promise ("Ce thread va changer ta façon de voir…" / "This thread will change how you see…").
- Also ban: hindsight regret hooks ("J'aurais aimé savoir ça avant" / "I wish I knew this earlier"); Excel-grid lists (5+ bullets of near-identical length); decorative emoji suffixes on titles/bullets (🔥🚀💡).
...
- "3 lessons" / numbered moral lists, intro → anecdote → bullets → moral → rhetorical question arcs · see school-connector overlap above.
...
- Interchangeable geo/sector filler (geo_sector_filler · structural, not lexicon): if swapping "Mexique / Amérique latine" (or SaaS↔other sector) for any other market still "works", the opener is too template · rewrite with a non-compressible stake. Detector: opener window (~320 chars / 2 paragraphs) + geo/sector slot + template frame minus digit/%/deadline/money anchor (see TELLS-CATALOG #43).
