...
export async function POST(request: Request) {
 ...
 const gate = await requireGenerationAccess(userId);
 if (!gate.ok) {
 ...
 const llm = await resolveContentRouteLlm(userId, body.llm, gate.access);
 ...
 if (allMissingEmojis) {
 const retryRaw = await chatCompletionJson(
 llm,
 [
 {
 role: "system",
