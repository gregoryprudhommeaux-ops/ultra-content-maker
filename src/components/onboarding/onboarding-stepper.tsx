...
function StepIndicator({ status, index }: { status: StepVisualStatus; index: number }) {
  if (status === "complete") {
    return (
  ...
  if (status === "current") {
    return (
      <span className="flex h-8 w-8 items-center justify-center rounded-full border-2 border-ns-primary bg-ns-hero text-xs font-bold text-ns-primary ring-2 ring-ns-primary/30">
        {index}
      </span>
    );
  }
    ...
    <span
      className={`flex h-8 w-8 items-center justify-center rounded-full border text-xs font-bold ${
        status === "available"
          ...
          : "border-ns-alternate/60 bg-ns-brand-light text-ns-secondary/50"
      }`}
    >
      {index}
    </span>
